...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In terms of data gathering, the Yelp API does not support gathering data by searching a whole county for the U.S.; it obligates us to search a city. Additionally, the API has a limit of 50 queries. Thus, we collected the top 20 most popular food establishments in the most populated city of each of the 83 counties (indicated in the commented code below) in the state of Michigan. We will search with "food" as the term of interest and the "location" as the most populated city within the specific county. Then, we will append all 83 datasets to have a full dataset of the most popular food establishments, cuisine type of each food establishment, and city of each food establishment for the state of Michigan.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;One of the columns of interest in our dataset (all_establishments), 'categories', requires some clean-up with formatting. Additionally, we will need to place the categories into new appropriate buckets to make analysis relevant. Thus, we will reformat that column in order to make it more understandable and clear.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Our cleaned data has 1570 observations since the most populated area of certain counties have less than 20 food establishment options on Yelp.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;The data from Yelp provides us information regarding location (city, state, longtitude, latitude). However, we are interested in studying counties within the state of Michigan. Thus, we will create an object that will map the food establishment's city to its associated county.
```{r}
