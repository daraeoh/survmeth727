...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;After removing the "Non-restaurant", "Grocery", and "Other" categories from all_establishments, our final dataset of cuisines and counties includes 1476 unique observations.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;We took the average of the male and female obesity rates and created a new column called “obesity_avg” as our dependent variable of interest, as shown below. We will refer to this variable as “aggregate rate of obesity.”
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;In our exploratory analysis, we created a basic barplot for `categories` to get an idea of the prevalence of different types of the most popular cuisines on Yelp in the state of Michigan.
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;According to our barplot, American, Bars/Breweries, Cafes/Dessert, and Pizza are the most popular types of cuisines on Yelp for Michigan.

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Additionally, [blah blah blah talk about the male/female plots]
...
master %>%
  lm(obesity_avg ~ factor(categories), data = .) %>%
  tidy()
```
...
```{r}
#aggregate rate of obesity for top 10 county categories arranged by 
#highest average aggregate rate of obesity segmented by cuisine categories

master %>%
  ggvis(x = ~categories, y = ~obesity_avg, fill = ~county) %>%
  layer_points(data = subset(master, county == "Saginaw"), fill = ~"Saginaw") %>%
  layer_points(data = subset(master, county == "Genesee"), fill = ~"Genesee") %>%
  layer_points(data = subset(master, county == "Lake"), fill = ~"Lake") %>%
  layer_points(data = subset(master, county == "Gratiot"), fill = ~"Gratiot") %>%
  layer_points(data = subset(master, county == "Ionia"), fill = ~"Ionia") %>%
  layer_points(data = subset(master, county == "Calhoun"), fill = ~"Calhoun") %>%
  layer_points(data = subset(master, county == "Arenac"), fill = ~"Arenac") %>%
  layer_points(data = subset(master, county == "Monroe"), fill = ~"Monroe") %>%
  layer_points(data = subset(master, county == "Oceana"), fill = ~"Oceana") %>%
  layer_points(data = subset(master, county == "Wayne"), fill = ~"Wayne") %>%
  add_axis("y", title_offset = 40, title = "Aggregate Rate of Obesity (%)") %>%
  add_axis("x", title_offset = 40, title = "", properties = axis_props(
    labels = list(
      angle = 270, 
      align = "right"),
      title = list(fontSize = 15))) %>%
  add_legend("fill", title = "Top 10 Counties with Highest Average Aggregate Rate of Obesity") 
```
